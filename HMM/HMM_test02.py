...
test_end_date = '2021-03-10'
all_data = hist_prices = myHMM.parseStockPrices(test_start_date, test_end_date, 'sz50')
...
for n in xrange(1, N - test_window, test_window):
    ...
    if ((all_data[-n - test_window - 1, 1] - all_data[-n - test_window, 1]) >= 0 and prediction == 1) or (
            (all_data[-n - test_window - 1, 1] - all_data[-n - test_window, 1]) <=0 and prediction == 0):
        num_correct += 1.0
print(num_correct / num_tests,num_tests)