...
test_end_date = '2021-03-08'
def get_socre(symbol):

    all_data = hist_prices = myHMM.parseStockPrices(test_start_date, test_end_date, symbol)
...
if __name__ == '__main__':
    ...
    with open("result.txt",mode='a') as f:
        for i in symbol_list:
            ...
            socre = get_socre(i)
            if socre[-1] >=0.85:
                print("%s\t  %d %d %f flag"%(i,socre[0],socre[1],socre[-1]))
                f.write("%s\t%f"%(i,socre[-1]))
